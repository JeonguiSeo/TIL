...
- **Introduction**
    ...
    - 주의사항
        ...
        - Upper/lower case 구분
            - True, False는 반드시 첫 문자가 upper case
        - Double/single quote는 구분하지 않음
        - Pycharm에서 오류 발생 시, Process finished with exit code ≠ 0
            - 오류는 console 최상단에 출력
    - Tip
    ...
    - Variable
        - Data 저장용
            ...
            - 집합형
        - Variable = value
            - 변수 자체에 값을 지정하는 것이 아닌, 값이 저장된 주소를 지정하는 것 (Reference를 가짐)
            - 즉, python에서는 모든 variable이 reference variable
            - Python에만 해당하는 특징
        - Simultaneous assignment (동시 할당
            ...
            # 반드시 variable과 value의 개수가 일치해야 함
            name, age, address = "홍길동",20,"서울"  # tuple이나 list 형태로 입력하더라도 각각 할당
    ...
    - 표준 입력 - input
        ...
        age = input('나이 입력')  # 31  # input에서 키보드로 입력하는 모든 명령은 '문자형'으로 인식
        grade = int(input('학년 입력'))
        
        print(name, age)
        
        print(name, int(age) + 1, grade + 1)  # 숫자형으로 변경
        ```
    ...
    - Format
        ...
        # 4. unpacking - 문자열/리스트 (집합형 data를 분할하여 출력 w/'*')
...
- **집합형 데이터**
    - String data
        ...
        - Unicode / bytes
            ...
            - Bytes string : 네트워크를 이용하는 경우, bytes string으로 처리
